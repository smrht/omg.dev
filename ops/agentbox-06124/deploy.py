...
h=pathlib.Path.home();w=h/'.cache/agent-tmp/omg-update-06124';live=h/'omg';private=h/'.local/lib/omg-private';new=private/'releases/06124-agentbox-20260924';backups=h/'.local/state/omg-update-backups/update-06124';old=private/'releases/06117-1024-desktop-20260923'
def run(*args):subprocess.run(list(map(str,args)),check=True)
def point(target):
 p=private/'current.next-06124';p.symlink_to(target);p.replace(private/'current')
def failed():
 result=[]
...
if mode=='preflight':
 ...
 print('PINNED_06124_READY')
elif mode=='restart':
 version=json.loads((live/'package.json').read_text())['version']
 assert version in ['0.6.117','0.6.124']
 point(new if version=='0.6.124' else old)
