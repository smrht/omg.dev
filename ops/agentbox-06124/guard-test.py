...
h=pathlib.Path.home();r=h/'.cache/agent-tmp/omg-update-06124/pristine';guard=h/'.local/lib/omg-private/releases/06124-agentbox-20260924/preserve.py'
...
p=r/'package.json';original=p.read_text();value=json.loads(original);value['version']='0.6.125';p.write_text(json.dumps(value))
try:
 out=subprocess.run(['python3',str(guard),str(r),'--apply'],capture_output=True,text=True)
