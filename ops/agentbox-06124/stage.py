"""Stage the reviewed 0.6.124 candidate and its immutable private snapshot.

candidate = official 0.6.124 release + Agentbox source deltas + new build,
...
h=pathlib.Path.home(); r=h/'.cache/agent-tmp/omg-port-06124/tree'; w=h/'.cache/agent-tmp/omg-update-06124'; pristine=w/'pristine'; candidate=w/'candidate'
snap=h/'.local/lib/omg-private/releases/06124-agentbox-20260924'
RELEASE_SHA='3e6fb3385dc989eda2a4891f561d2ffe8e35117deaeffbcab1a633df04b0ffbd'
...
assert json.loads((pristine/'package.json').read_text())['version']=='0.6.124'
assert json.loads((r/'package.json').read_text())['version']=='0.6.124'
assert not candidate.exists() and not snap.exists()
shutil.copytree(pristine,candidate,symlinks=True)
paths=subprocess.check_output(['git','-C',str(r),'diff','v0.6.124','--name-only'],text=True).splitlines()
review=set()
for rel in paths:
...
(snap/'manifest.json').write_text(json.dumps({'version':'0.6.124','files':files},indent=2)+'\n')
shutil.copy2(r/'ops/agentbox-06124/preserve.py',snap/'preserve.py')
