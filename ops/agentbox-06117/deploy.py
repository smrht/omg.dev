...
h=pathlib.Path.home();w=h/'.cache/agent-tmp/omg-update-06117';live=h/'omg';private=h/'.local/lib/omg-private';new=private/'releases/06117-1021-20260923';backups=h/'.local/state/omg-update-backups/update-06117';old=private/'releases/0698-quiet-routines-20260923'
def run(*args):subprocess.run(list(map(str,args)),check=True)
def point(target):
 p=private/'current.next-1021';p.symlink_to(target);p.replace(private/'current')
def failed():
 result=[]
...
if mode=='preflight':
 ...
 print('PINNED_06117_READY')
elif mode=='restart':
 version=json.loads((live/'package.json').read_text())['version']
 assert version in ['0.6.98','0.6.117']
 point(new if version=='0.6.117' else old)
