...
Publication has NOT run. Issue1023 stays open until live QA passes.
...
6. Verify baseline with `OMG_UPDATE_BASELINE_DIR=$HOME/.local/state/omg-update-backups/model-picker-1023 python3 ~/.local/state/omg-update-backups/model-picker-1023/state.py verify`. Compare OMG MainPID/start time against service-before.txt. Live browser must verify picker/favorites/search/Flash/thinking/usage/composer at mobile and desktop widths, preserving test preferences and submitting no chat. A running process or HTTP200 is insufficient.
