...
with (h/'.local/state/omg-update-backups/.update.lock').open('a') as lock:
 ...
 changed=[];pointer=base/'current.next-desktop'
 try:
  for rel in plan:
   target=live/rel;target.parent.mkdir(parents=True,exist_ok=True);tmp=target.with_name(target.name+'.desktop-tmp');shutil.copy2(new/'files'/rel,tmp);tmp.replace(target);changed.append(rel)
  ...
  raise
 print('DESKTOP_LAYER_ACTIVATED',len(changed),'files; service unchanged; previous',old)
