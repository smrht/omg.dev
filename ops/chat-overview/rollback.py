...
h=pathlib.Path.home();base=h/'.local/lib/omg-private';live=h/'omg';old=base/'releases/06117-1021-menu-20260923'
with (h/'.local/state/omg-update-backups/.update.lock').open('a') as lock:
 fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
 current=(base/'current').resolve();assert current.name=='06117-1022-overview-20260923'
 ...
 pointer=base/'current.rollback-overview';pointer.symlink_to(old);pointer.replace(base/'current')
 print('PREVIOUS_OVERVIEW_RESTORED_NO_RESTART')
