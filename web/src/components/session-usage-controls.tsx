...
export function SessionUsageControls({ row, request, onChanged }: {
  ...
  async function inspect(force = false) {
    ...
    try { setPreview(await request('/api/server/session-usage/preview', { key: row.key, force })); }
  ...
  async function confirm() {
    ...
    try {
      ...
      setPreview(null);
      onChanged(`${result.closed ? 'Sessie gesloten. ' : ''}${result.stopped} ${result.stopped === 1 ? 'proces gestopt' : 'processen gestopt'}, ${Math.round(result.releasedBytes / 1048576)} MB vrijgemaakt. ${result.remaining === 0 ? 'Geen processen over.' : `${result.remaining} ${result.remaining === 1 ? 'proces blijft' : 'processen blijven'} staan.`}`);
    } catch (e) {
  ...
  return <>
    ...
    <AlertDialog open={!!preview} onOpenChange={open => { if (!open && !pending) setPreview(null); }}>
      <AlertDialogContent className="flex max-h-[90dvh] flex-col gap-3 overflow-y-auto">
        <AlertDialogTitle>{preview?.plan.force ? 'Restprocessen geforceerd stoppen' : label}?</AlertDialogTitle>
        <AlertDialogDescription>{row.title ?? preview?.plan.title}. Gesprek en bestanden blijven bewaard. {preview?.plan.force ? 'Deze processen worden direct beëindigd. Niet-opgeslagen werk kan verloren gaan. De werkmap koppelt ze aan dit gesprek; dat bewijst niet dat ze nergens anders nodig zijn.' : 'Alleen de hieronder goedgekeurde processen krijgen een stopverzoek.'}</AlertDialogDescription>
        ...
        <ul className="min-h-0 max-h-[40dvh] overflow-y-auto divide-y text-sm">
          {preview?.plan.targets.map(t => <li key={t.pid} className="py-2 break-words">
            ...
            <span className="block text-muted-foreground">{t.reason ? `Blijft staan: ${t.reason}` : preview?.plan.force ? 'Wordt geforceerd gestopt' : 'Wordt netjes gestopt'}</span>
          </li>)}
        </ul>
        <p className="text-xs text-muted-foreground">{preview?.plan.force ? 'Controle is één minuut geldig. Bekende actieve sessies, browsers en gedeelde diensten blijven beschermd.' : 'Controle is één minuut geldig. Een proces dat niet stopt blijft staan; geen harde stop.'}</p>
        {!row.live && !preview?.plan.force && <Button variant="outline" className="min-h-11" disabled={pending} onClick={() => void inspect(true)}>Geforceerd stoppen controleren</Button>}
        <div className="flex shrink-0 flex-wrap justify-end gap-2">
          <Button variant="outline" className="min-h-11" disabled={pending} onClick={() => setPreview(null)}>Annuleren</Button>
          <Button variant="destructive" className="min-h-11" disabled={pending || !!preview?.plan.blocked || !preview?.plan.targets.some(t => !t.reason)} onClick={() => void confirm()}>{pending ? 'Stoppen…' : preview?.plan.force ? 'Ja, geforceerd stoppen' : 'Bevestigen en stoppen'}</Button>
        </div>
      </AlertDialogContent>
    </AlertDialog>
  </>;
