...
export type AutoAgent = {
  ...
  tools?: string[];
  // Quiet routine: finished work and a valid no-op are not findings. The run
  // only files something the owner must resolve (a blocker, a failed step),
  // and low-severity "here is what I did" rows are dropped before filing. For
  // rows that fire every hour or two and mostly succeed — an owner does not
  // want a push per successful tick.
  quiet?: boolean;
  lastRunAt?: number;
};
...
export async function saveAutoAgent(input: {
  ...
  const agent = await mutateAutoAgents((list) => {
    ...
    const saved: AutoAgent = {
      ...
      quiet: input.quiet ?? existing?.quiet,
