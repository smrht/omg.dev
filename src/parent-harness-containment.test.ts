...
describe("managed parent harness containment (issue 521)", () => {
  ...
  test("Linux isolation cannot be disabled by the legacy opt-out", () => {
    const result = spawnManagedAisdkSession({
      name: "lfg-parenty",
      cwd: root,
      model: "opus",
    ...
    expect(result.ok).toBe(true);
    const launch = JSON.parse(readFileSync(capture, "utf8")) as { cmd: string[] };
    if (isLinux) expect(launch.cmd[0]).toContain("systemd-run");
    else expect(launch.cmd[0]).not.toContain("systemd-run");
